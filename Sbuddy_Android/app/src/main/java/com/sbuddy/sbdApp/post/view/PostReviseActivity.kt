...
class PostReviseActivity : AppCompatActivity() {
    private lateinit var binding: ActivityPostReviseBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_post_revise)
    ...
    fun setObserve() {
        ...
        postViewModel.detail.observe(this) { detail ->
            binding.titleEdit.setText(detail.title)
            binding.contentEdit.setText(detail.content)

        }

        postViewModel.updateFinished.observe(this){
            if(it){
                finish()
            }
        }
    ...
    fun revise(){
        if(postViewModel.selectedImageUri.value != null){
            postViewModel.revise(UploadUtil.uriToFile(this, postViewModel.selectedImageUri.value!!), binding.titleEdit.text.toString(), binding.contentEdit.text.toString(), position )
        }else{
            postViewModel.revise(null, binding.titleEdit.text.toString(), binding.contentEdit.text.toString(), position )
        }
    }
